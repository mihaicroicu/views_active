...
def read_opt(seed, active_learner, adam_lr, rounds):
    """Read options for training."""
    print(f"{seed=}, {active_learner=}, {adam_lr=}, {rounds=}")
    return seed, active_learner, adam_lr, rounds

seed, active_learner, adam_lr, rounds = read_opt(standalone_mode=False)
...
for epoch in range(num_epochs):
    ...
    pd.DataFrame({'seed':[seed],'epoch':[epoch],'lr':[adam_learning_rate],'bs':[active_batch_size],
             'ap':[ap], 'auroc':[auroc], 'acc':[acc['accuracy']]}).to_csv('rexp/agg.csv',mode='a+')
    ...
    p = likely_cands['xscore']
    likely_cands['xscore'] = -(p * np.log2(p) + (1 - p) * np.log2(1 - p))

    new_iter = likely_cands.sample(n=active_batch_size, weights='xscore')
    active_seed = pd.concat([new_iter, active_seed])
    print (f"At {epoch=} dataset size is {active_seed.shape[0]} of which positives: {active_seed.label.sum()}")
    train_dataloader = df_to_torch(active_seed, balance=True)

    
labels, probas, acc = get_predictions(eval_dataloader)

fname = f"rexp/experiment_{seed}_batch_{active_batch_size}_lr_{adam_learning_rate}.parquet"
