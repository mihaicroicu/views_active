...
@click.option('--pol',default=False,help='Politics or religion')
@click.option('--hf_model', default='conflibert', help="Pretrained model to active train")
def read_opt(seed, active_learner, adam_lr, rounds, pol, hf_model):
    """Read options for training."""
    print(f"{seed=}, {active_learner=}, {adam_lr=}, {rounds=}, {pol=}, {hf_model=}")
    return seed, active_learner, adam_lr, rounds, pol, hf_model

seed, active_learner, adam_lr, rounds, pol, hf_model = read_opt(standalone_mode=False)
#exit(1)
...
if not pol:
    path_rel_data = '/mimer/NOBACKUP/groups/snic2019-3-404/text_jocke_marisol/data_rel.parquet'
    pref='rel'
else:
...
torch.save(model.state_dict(), f'single_res/{hf_model}_{pref}_tensors.pt')
...
torch.save(model, f"single_res/{hf_model}_{pref}_model.pt")
