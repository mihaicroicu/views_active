...
@click.option('--rounds',default=15, help='Active Learning Rounds')
...
def read_opt(seed, active_learner, adam_lr, rounds, dset, hf_model):
    """Read options for training."""
    print(f"{seed=}, {active_learner=}, {adam_lr=}, {rounds=}, {dset=}, {hf_model=}")
    return seed, active_learner, adam_lr, rounds, dset, hf_model

seed, active_learner, adam_lr, rounds, dset, hf_model = read_opt(standalone_mode=False)
#exit(1)
...
dset = dset.lower().strip()
if dset == 'rel':
    path_rel_data = 'data_rel.parquet'
    pref='rel'
if dset == 'pol':
    path_rel_data = '/mimer/NOBACKUP/groups/snic2019-3-404/text_jocke_marisol/data_elections_new.parquet'
    pref='pol'
if dset == 'infra':
    path_rel_data = './infra.parquet'
    pref='infra'
print (f"{pref=}")

print (f"{pretrained_model=}, {path_rel_data=}")
...
r_test = rel.loc[~rel.index.isin(r_train.index)]
...
prob_pos = r_train.sample(n=active_batch_size, weights=rel.xscore**4)
...
for epoch in range(num_epochs):
    ...
    #         'ap':[ap], 'auroc':[auroc], 'acc':[acc['accuracy']]}).to_csv(f'single_res/{pref}_agg.csv',mode='a+')
...
torch.save(model.state_dict(), f'single_res/{hf_model}_{pref}_{seed}_tensors.pt')

labels, probas, acc = get_predictions(eval_dataloader)

ap, auroc = roc_pr_scorer(labels, probas)
print (f"{epoch=} :: {ap=}, {auroc=}, {acc=}")
pd.DataFrame({'seed':[seed],'epoch':[epoch],'lr':[adam_learning_rate],'bs':[active_batch_size],'ap':[ap], 'auroc':[auroc], 'acc':[acc['accuracy']]}).to_csv(f'single_res/{hf_model}_{pref}_agg.csv',mode='a+')

torch.save(model, f"single_res/{hf_model}_{pref}_{seed}_model.pt")

fname = f"single_res/ex_{hf_model}_{pref}_{seed}_batch_{active_batch_size}_lr_{adam_learning_rate}.parquet"
pd.DataFrame({'actuals':labels,'preds':probas}).to_parquet(fname)
